...
void ExtDef(Node *node){
    ...
    //FunDec是函数头，CompSt表示函数体
        ...
        if(strcmp(node->first_son->follow->follow->name,"CompSt")==0){
            ...
            CompSt(node->first_son->follow->follow,specifier);
        }
        //不做声明任务(3.1),故认为是定义
        else{
            FunDec(node->first_son->follow,specifier,DEFIN);
...
Type StructSpecifier(Node *node)
{
    ...
    if(node->num_child == 4) {
        if(strcmp(first->follow->name, "OptTag") == 0)
            optTag = first->follow;
        else if(strcmp(first->follow->follow->name, "DefList") == 0)
            defList = first->follow->follow;
    }
    else if(node->num_child == 5) {
        optTag = first->follow;
        defList = first->follow->follow->follow;
    }
...
FieldList VarDec(Node *type,Type spec) {
    ...
    if(strcmp(first->name, "ID") == 0) {
        ...
        // printf("NULL? -> %d\n", first == NULL);
        // assert(first != NULL);
        field->name = strdup(type->first_son->token);

        return field;
    }
    else if(strcmp(first->name, "VarDec") == 0) {
...
void Dec(Node *node,Type spec) {
    ...
    if(ifexist(varField->name,varField->scope_id)!=NULL){
        ...
        //printf("insert %s %d\n",varField->name, varField->scope_id);
        insert(varField);
    }
}
...
//         | FLOAT             1.0
    ...
    if((node->num_child == 3 || node->num_child == 4) && strcmp(node->first_son->name, "ID") == 0){
//         | ID LP Args RP     a(b)
//         | ID LP RP          a()
        //找函数名
        node->flag = 1;
